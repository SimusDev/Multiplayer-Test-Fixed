...
func send_property_to_peer_synced_data(node: Node, property: String, value: Variant = null, peer: int = SD_MultiplayerSingleton.HOST_ID, reliable: bool = false) -> void:
	...
	var serialized_value: Variant = SD_MPDataCompressor.serialize_data(value)
	if reliable:
...
func _recieve_property_to_synced_data_reliable(node_path: String, property: String, ser_value: Variant) -> void:
	var deserialized_value: Variant = SD_MPDataCompressor.deserialize_data(ser_value)
	...
	if node:
		...
		if multiplayer:
			property_recieved.emit(node, property, deserialized_value, multiplayer.get_remote_sender_id())
