...
func rpf(peer: int, property: SD_NetSyncedProperty, apply_changes: bool = false) -> void:
	SD_Network.call_func_on(peer, spti, [SD_Network.get_unique_id(), _data.get_list().find(property), apply_changes], property.callmode, property.channel)
...
func spti(peer: int, property_id: int, apply_changes: bool = false) -> void:
	...
	SD_Network.call_func_on(peer, rp, [property_id, send_data, apply_changes], property.callmode, property.channel)
