...
func emit_smoke(_wheel):
	if _wheel:
		...
		smoke_transform.origin = _wheel.last_collision_point
		emit_particle(smoke_transform,  _wheel.global_transform.basis * ((_wheel.local_velocity * 0.2) - (Vector3.FORWARD * _wheel.spin * _wheel.tire_radius * 0.2)) * self.global_transform.basis, Color.WHITE, Color.WHITE, 5) #EMIT_FLAG_POSITION + EMIT_FLAG_VELOCITY)
