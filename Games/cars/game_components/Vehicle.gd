...
@export var front_left_wheel : Wheel = null
@export var front_right_wheel : Wheel = null
@export var rear_left_wheel : Wheel = null
@export var rear_right_wheel : Wheel = null
...
func _physics_process(delta : float) -> void:
	...
	if ui:
		ui.get_node("rpm_label").text = "engine rpm: " + str(motor_rpm)
		ui.get_node("gear_label").text = "current gear: " + str(current_gear)
	
	idle_rpm = randf_range(750, 850)
...
func manual_shift(count : int) -> void:
	...
	if clutch_amount > 0.4 or motor_rpm < 300:
		shift(count)
...
func calculate_center_of_gravity(front_distribution : float) -> Vector3:
	var rear_right_wheel_position = Vector3.ZERO
	var rear_left_wheel_position = Vector3.ZERO
	var front_left_wheel_position = Vector3.ZERO
	var front_right_wheel_position = Vector3.ZERO
	
	if rear_right_wheel: rear_right_wheel_position = rear_right_wheel.position
	if rear_left_wheel: rear_right_wheel_position = rear_left_wheel.position
	if front_right_wheel: front_right_wheel_position = front_right_wheel.position
	if front_left_wheel: front_left_wheel_position = front_left_wheel.position
	
	front_axle_position = front_left_wheel_position.lerp(front_right_wheel_position, 0.5)
	rear_axle_position = rear_left_wheel_position.lerp(rear_right_wheel_position, 0.5)
